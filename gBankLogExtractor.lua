...
local function CollectVisibleLogLines()
    ...
    -- We sometimes get nil or non-string entries??, just clean those out
    local clean = {}
...
SlashCmdList["GB"] = function(msg)
    ...
    Print("Starting " .. interval .. "-second guild bank loop...")

    f:SetScript("OnUpdate", function()
        local now = GetTime()
        if now - lastTime >= interval then
            Print(interval .. " seconds passed.")

            -- Go to log tab
            GuildBankFrameBottomTab_OnClick(2)
